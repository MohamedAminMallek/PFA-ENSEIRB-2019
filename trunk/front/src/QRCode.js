...
var QrcodeVue = {
  ...
  methods: {
    render: function render() {
      ...
      if (pageSetUp[0] == -1)
        pageSetUp = [value.length, 1];

      var _size = size >>> 0; // size to number
      // We'll use type===-1 to force QRCode to automatically pick the best type
      let currentPagePosition = [0,0];

      var canvas = this.$refs['qrcode-vue'];
      var ctx = canvas.getContext('2d');
      var scale = (window.devicePixelRatio || 1) / getBackingStorePixelRatio(ctx);
      canvas.width = _size * scale * pageSetUp[0] + ((pageSetUp[0] + 1) * margin);
      canvas.height = _size * scale * pageSetUp[1] + ((pageSetUp[1] + 1) * margin);

      value.forEach(function (currentValue) {

          var qrCode = new QRCode_1(-1, ErrorCorrectLevel[level]);
          qrCode.addData(toUTF8String(currentValue));
          qrCode.make();
          ...
          cells.forEach(function (row, rdx) {
            row.forEach(function (cell, cdx) {
              ...
              ctx.fillRect(Math.round((cdx * tileW)+(currentPagePosition[0] * (_size + margin) + margin)),
                           Math.round((rdx * tileH)+(currentPagePosition[1] * (_size + margin) + margin)),
                           w, h);
            });
          });

          currentPagePosition[0] += 1;
          if (currentPagePosition[0] >= pageSetUp[0]) {
            currentPagePosition[1] += 1;
            currentPagePosition[0] = 0;
          }
